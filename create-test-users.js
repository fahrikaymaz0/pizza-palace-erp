...
try {
  ...
  const profilesTable = db.prepare(`
    SELECT name FROM sqlite_master 
  ...
  for (const userData of testUsers) {
    try {
      ...
      console.log(`✅ Kullanıcı oluşturuldu: ${userData.email} (ID: ${userId})`);

    } catch (error) {
      console.error(`❌ Kullanıcı oluşturma hatası (${userData.email}):`, error.message);
    }
  }
