...
export class PayTRPaymentAPI {
  ...
  async createPaymentLink(paymentData: any) {
    const { amount, email, user_name, user_address, user_phone, user_basket } =
      paymentData;
  ...
  async checkTransactionStatus(merchantOid: string) {
    try {
      console.log(
        '🔍 PayTR Status Inquiry API ile işlem durumu sorgulanıyor...'
      );
  ...
  isTestEnvironment() {
    return (
      this.config.paytr.merchantId === 'YOUR_MERCHANT_ID' ||
  ...
  async processPayTRPayment(paymentData: any) {
    const { cardNumber, expiryMonth, expiryYear, cvv, cardHolder, amount } =
      paymentData;
  ...
  handlePaymentError(error: any) {
    ...
    return {
      ...
      error: error.message,
      friendlyMessage:
        errorMap[error.message.split(':')[0]] || 'PayTR ödeme işlemi başarısız',
    };
  }
}
...
export async function checkTestTransactionStatus(merchantOid: string) {
  ...
  try {
    console.log(
      '🔍 PayTR Status Inquiry API test işlem durumu sorgulanıyor...'
    );
    console.log('Merchant OID:', merchantOid);
    ...
    if (result.success) {
      ...
      console.log(
        `Tutar: ₺${(parseInt(result.total_amount) / 100).toFixed(2)}`
      );
