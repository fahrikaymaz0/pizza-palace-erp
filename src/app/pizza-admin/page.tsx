...
export default function PizzaAdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    'pending' | 'approved' | 'delivery' | 'completed' | 'cancelled'
  >('pending');
  const [cancelledOrders, setCancelledOrders] = useState<Order[]>([]);
  const [showCancelledNotification, setShowCancelledNotification] =
    useState(false);
    ...
    try {
      ...
      if (activeTab === 'pending') {
        if (pending.length > 0) {
          ...
          const currentSelectedStillPending =
            selectedOrder &&
            pending.find((order: any) => order.id === selectedOrder.id);
  ...
  const updateOrderStatus = async (orderId: number, newStatus: number) => {
    try {
      ...
      console.log(
        `✅ Sipariş #${orderId} durumu ${newStatus} olarak güncellendi`
      );
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div className="bg-blue-50 rounded-lg p-4">
                  ...
                  <div>
                    ...
                    <PhoneDisplay phone={selectedOrder.phone} />
                  </div>
                </div>
                {selectedOrder.notes && (
            ...
            <div className="bg-white rounded-lg shadow p-8 text-center">
              ...
              <h3 className="mt-2 text-sm font-medium text-gray-900">
                {selectedOrder && selectedOrder.status !== 0
                  ? 'Seçili sipariş artık onay beklemeyen durumda'
                  : 'Sipariş seçilmedi'}
              </h3>
              <p className="mt-1 text-sm text-gray-500">
                {selectedOrder && selectedOrder.status !== 0
                  ? 'Lütfen başka bir onay bekleyen sipariş seçin.'
                  : 'Onay bekleyen bir sipariş seçin.'}
              </p>
            </div>
          )
        )}
        ...
        {activeTab === 'approved' && (
          <div className="bg-white rounded-lg shadow">
            <div className="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-green-50 to-emerald-50">
              ...
              <p className="text-sm text-gray-600">
                {orders ? orders.filter(o => o.status === 1).length : 0} sipariş
                onaylandı
              </p>
            </div>
        ...
        {activeTab === 'delivery' && (
          <div className="bg-white rounded-lg shadow">
            <div className="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-purple-50 to-indigo-50">
              ...
              <p className="text-sm text-gray-600">
                {orders ? orders.filter(o => o.status === 2).length : 0} sipariş
                kuryede
              </p>
            </div>
            ...
            {!orders || orders.filter(o => o.status === 2).length === 0 ? (
              <div className="p-8 text-center">
                ...
                <p className="mt-1 text-sm text-gray-500">
                  Kuryede olan siparişler burada görünecek.
                </p>
              </div>
            ) : (
              <div className="divide-y divide-gray-200">
        ...
        {activeTab === 'completed' && (
          <div className="bg-white rounded-lg shadow">
            <div className="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-cyan-50">
              ...
              <p className="text-sm text-gray-600">
                {orders ? orders.filter(o => o.status === 3).length : 0} sipariş
                tamamlandı
              </p>
            </div>
