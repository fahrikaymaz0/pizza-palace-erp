...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(
      `✅ [${requestId}] Input validation geçti, SQLite bağlantısı kuruluyor...`
    );
    ...
    try {
      ...
      console.error(
        `❌ [${requestId}] Kullanıcı kontrol hatası:`,
        userCheckError
      );
      return NextResponse.json(
        {
    ...
    console.log(
      `✅ [${requestId}] Email müsait, doğrulama kodu oluşturuluyor...`
    );
    ...
    try {
      ...
      console.log(
        `📧 [${requestId}] Doğrulama kodu oluşturuldu:`,
        verificationCode
      );
    } catch (codeGenError) {
    ...
    try {
      ...
      console.error(
        `⚠️ [${requestId}] Email gönderme hatası (devam ediliyor):`,
        emailError
      );
    ...
    if (!emailSent) {
      console.log(
        `⚠️ [${requestId}] Email gönderilemedi ama kod kaydediliyor:`,
        email
      );
    }
