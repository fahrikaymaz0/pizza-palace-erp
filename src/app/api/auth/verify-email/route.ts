...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      console.error(`❌ [${requestId}] SQLite Database başlatma hatası:`, dbInitError);
      return createErrorResponse(
    ...
    if (existingUser) {
      ...
      database.prepare('UPDATE user_profiles SET email_verified = 1 WHERE user_id = ?').run(user.id);
      ...
      try {
        ...
        const result = insertUser.run(email.toLowerCase(), hashedPassword, userData.name);
        ...
        console.log('✅ Kullanıcı başarıyla kaydedildi ve email doğrulandı:', email);
      } catch (error) {
    ...
    console.log(`✅ [${requestId}] Email doğrulandı ve kullanıcı giriş yaptı:`, email);
