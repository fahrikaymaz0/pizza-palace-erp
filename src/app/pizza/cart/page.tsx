...
export default function CartPage() {
  ...
  const calculateDiscount = () => {
    ...
    switch (selectedCoupon.type) {
      ...
      case 'buy_x_get_y':
        if (selectedCoupon.minQuantity && totalQuantity < selectedCoupon.minQuantity) {
          return 0;
        }
  ...
  const applyCoupon = () => {
    ...
    if (coupon) {
      ...
      if (coupon.minAmount && getSubtotal() < coupon.minAmount) {
        alert(`Bu kupon için minimum ${coupon.minAmount}₺ tutarında alışveriş gereklidir!`);
        return;
      }
  ...
  const handlePaymentSubmit = async () => {
    ...
    try {
      ...
      const paymentResult = await makeTestPayment(paymentData, getFinalTotal()) as any;
      ...
      if (paymentResult && paymentResult.success) {
        ...
        if (response.ok) {
          ...
          if (response.status === 401) {
            ...
            alert(`Sipariş oluşturulurken hata oluştu: ${errorData.error || 'Bilinmeyen hata'}`);
          }
        }
      } else {
        console.error('❌ Test ödeme başarısız:', paymentResult);
        alert(`Ödeme işlemi başarısız: ${paymentResult.friendlyMessage || paymentResult.error}`);
      }
    } catch (error) {
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
              ...
              <div className="divide-y divide-gray-100">
                {cart.map((item) => (
                  <div key={item.id} className="p-6 flex items-center space-x-4 hover:bg-gray-50 transition-colors duration-200">
                ...
                <div>
                  ...
                  {phone && !validatePhoneNumber(phone) && (
                    <p className="text-red-500 text-sm mt-1 flex items-center">
                      ...
                      Geçerli bir Türkiye telefon numarası giriniz (5XX XXX XX XX)
                    </p>
                  )}
                </div>
                ...
                {selectedCoupon ? (
                  <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-xl p-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-semibold text-green-800">{selectedCoupon.name}</p>
                        <p className="text-sm text-green-600">{selectedCoupon.description}</p>
                      </div>
                      <button
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      <p className="text-sm font-semibold text-gray-700">Mevcut Kuponlar:</p>
                      {availableCoupons.map((coupon) => {
                        const isEligible = (!coupon.minQuantity || getTotalQuantity() >= coupon.minQuantity) &&
                                         (!coupon.minAmount || getSubtotal() >= coupon.minAmount);
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center justify-between">
                              <div className="flex-1">
                                <p className={`font-semibold ${isEligible ? 'text-green-800' : 'text-gray-500'}`}>
                                  {coupon.name}
                                </p>
                                <p className={`text-sm ${isEligible ? 'text-green-600' : 'text-gray-400'}`}>
                                  {coupon.description}
                                </p>
                              </div>
                  ...
                  <div className="space-y-3">
                    <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4">
                      <div className="flex items-center">
                        ...
                        <span className="text-yellow-800 font-medium">Sipariş vermek için giriş yapmanız gerekiyor</span>
                      </div>
                    </div>
                    <button
                    ...
                    <div className="bg-green-50 border border-green-200 rounded-xl p-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center">
                          ...
                          <span className="text-green-800 font-medium">Giriş yapıldı</span>
                        </div>
                        <button
                          onClick={refreshAuth}
                          ...
                          title="Durumu yenile"
                        >
                          <svg className={`w-4 h-4 ${authChecking ? 'animate-spin' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full border-2 rounded-xl px-4 py-4 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent transition-all duration-200 text-lg font-mono ${
                        paymentErrors.cardNumber ? 'border-red-500 bg-red-50' : 'border-gray-300 hover:border-gray-400'
                      }`}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full border-2 rounded-xl px-4 py-4 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent transition-all duration-200 text-lg ${
                        paymentErrors.cardHolder ? 'border-red-500 bg-red-50' : 'border-gray-300 hover:border-gray-400'
                      }`}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        maxLength={5}
                        className={`w-full border-2 rounded-xl px-4 py-4 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent transition-all duration-200 text-lg font-mono ${
                          paymentErrors.expiryDate ? 'border-red-500 bg-red-50' : 'border-gray-300 hover:border-gray-400'
                        }`}
                        placeholder="MM/YY"
                      />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        maxLength={4}
                        className={`w-full border-2 rounded-xl px-4 py-4 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent transition-all duration-200 text-lg font-mono ${
                          paymentErrors.cvv ? 'border-red-500 bg-red-50' : 'border-gray-300 hover:border-gray-400'
                        }`}
                        placeholder="123"
                      />
                      {paymentErrors.cvv && (
                  ...
                  <div className="bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl p-4 border border-gray-200">
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="border-t border-gray-300 pt-2 flex justify-between font-bold text-lg">
                        <span>Toplam:</span>
                        <span className="text-red-600">₺{getFinalTotal().toFixed(2)}</span>
                      </div>
                    </div>
                  </div>
