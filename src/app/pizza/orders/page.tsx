    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-red-50 to-orange-50 flex items-center justify-center">
        <div className="text-center bg-white p-8 rounded-2xl shadow-xl max-w-md w-full mx-4">
          ...
          <h2 className="text-xl font-bold text-gray-900 mb-2">Siparişleriniz Yükleniyor</h2>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-gray-600">Siparişlerinizi buradan takip edebilirsiniz</p>
              </div>
            </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
            ...
            <div className="flex items-center">
              ...
              <div className="ml-3">
                ...
                <div className="mt-2 text-sm text-green-700">
                  <p>Siparişiniz restoran tarafından onaylanmayı bekliyor. Durumunu aşağıdan takip edebilirsiniz.</p>
                </div>
              </div>
        ...
        {orders && Array.isArray(orders) && orders.length === 0 ? (
          <div className="text-center bg-white rounded-2xl shadow-xl p-12">
            ...
            <p className="text-gray-600 mb-8">İlk siparişinizi vermek için menüye göz atın!</p>
            <Link
              href="/pizza/menu"
          ...
          <div className="space-y-6">
            {orders && orders.map((order) => (
              <div key={order.id} className={`rounded-2xl shadow-xl border overflow-hidden transition-all duration-300 ${
                order.status === 0 ? 'bg-yellow-50 border-yellow-200' :
                order.status === 1 ? 'bg-green-50 border-green-200' :
                order.status === 2 ? 'bg-purple-50 border-purple-200' :
                order.status === 3 ? 'bg-blue-50 border-blue-200' :
                order.status === -1 ? 'bg-red-50 border-red-200' :
                'bg-white border-gray-100'
              }`}>
                {/* Order Header */}
                <div className={`p-6 border-b border-gray-100 ${
                  order.status === 0 ? 'bg-gradient-to-r from-yellow-50 to-orange-50' :
                  order.status === 1 ? 'bg-gradient-to-r from-green-50 to-emerald-50' :
                  order.status === 2 ? 'bg-gradient-to-r from-purple-50 to-indigo-50' :
                  order.status === 3 ? 'bg-gradient-to-r from-blue-50 to-cyan-50' :
                  order.status === -1 ? 'bg-gradient-to-r from-red-50 to-pink-50' :
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        <h3 className="text-lg font-bold text-gray-900">Sipariş #{order.id}</h3>
                ...
                <div className="p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        {order.phone && (
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="text-gray-600">{formatPhoneForDisplay(order.phone)}</span>
                          </div>
                        )}
                        {order.notes && (
                          <p className="text-gray-700">
                    ...
                    <div className="bg-green-50 rounded-xl p-4 border border-green-200">
                      ...
                      <div className="space-y-2 text-sm">
                        <p className="text-gray-700">
                          <span className="font-medium">Ödeme:</span> Kredi Kartı ile Ödendi
                        </p>
                        {order.payment && (
                          <>
                            <p className="text-gray-700">
                              <span className="font-medium">Kart:</span> •••• •••• •••• {order.payment.cardNumber.slice(-4)}
                            </p>
                            <p className="text-gray-700">
                              <span className="font-medium">Kart Sahibi:</span> {order.payment.cardHolder}
                            </p>
                          </>
                        )}
                        {order.transaction && (
                          <>
                            <p className="text-gray-700">
                              <span className="font-medium">İşlem No:</span> {order.transaction.transactionId}
                            </p>
                            <p className="text-gray-700">
                              <span className="font-medium">Banka:</span> {order.transaction.bank}
                            </p>
                            <p className="text-gray-700">
                              <span className="font-medium">Yetki Kodu:</span> {order.transaction.authCode}
                            </p>
                          </>
                        )}
                      </div>
