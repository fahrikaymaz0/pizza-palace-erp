...
export default function DataManagement({
  ...
  const loadData = async () => {
    try {
      ...
      const schemaResponse = await fetch(
        `/api/clients/${clientId}/tables/${tableName}`
      );
      ...
      const dataResponse = await fetch(
        `/api/clients/${clientId}/tables/${tableName}/data`
      );
  ...
  const handleDeleteData = async (id: any) => {
    ...
    try {
      const response = await fetch(
        `/api/clients/${clientId}/tables/${tableName}/data?id=${id}`,
        {
          method: 'DELETE',
        }
      );
    ...
    <div>
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-gray-600">{data.length} kayıt bulundu</p>
        </div>
        <button
                  ...
                  <div key={column.name}>
                    ...
                    {column.type === 'BOOLEAN' ? (
                      <input
                        ...
                        onChange={e =>
                          handleFormChange(column.name, e.target.checked)
                        }
                        className="mr-2"
                      />
                    ) : (
                      <input
                        ...
                        onChange={e =>
                          handleFormChange(column.name, e.target.value)
                        }
                    ...
                    <button
                      onClick={() =>
                        handleDeleteData(
                          row.id ||
                            row[
                              schema.find(col => col.primaryKey)?.name || 'id'
                            ]
                        )
                      }
