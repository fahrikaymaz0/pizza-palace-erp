...
const PayTRDirektAPI = () => {
  ...
  const autoLoadTestCard = (detectedCard: any) => {
    if (detectedCard.type === 'test' && detectedCard.card) {
      ...
      console.log(
        '🎯 PayTR Test Kartı Otomatik Yüklendi:',
        detectedCard.card.description
      );
    }
  };
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.error === 'ENV_ERROR') {
          alert(
            '⚠️ PayTR Environment Variables Ayarlanmamış!\n\n' +
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.is_simulated) {
          ...
          alert(
            `🎉 PayTR ${detectedCard?.brand || 'Test'} Kartı ile simüle edilmiş ödeme başarılı!\n\n` +
        ...
        if (result.error === 'ENV_ERROR') {
          errorMessage +=
            '⚠️ Environment Variables Ayarlanmamış!\n\n' +
            'Lütfen .env.local dosyasında PayTR bilgilerinizi ayarlayın.\n\n';
        }

        if (detectedCard) {
          errorMessage +=
            `🎯 PayTR Test Kartı Algılandı:\n` +
  ...
  const getApiStatusText = () => {
    switch (apiStatus) {
      case 'connected':
        return 'PayTR Direkt API Bağlantısı OK';
      case 'testing':
        return 'PayTR Direkt API Test Ediliyor...';
      default:
        return 'PayTR Direkt API Bağlantısı Yok';
    }
  };
              ...
              <div className="backdrop-blur-sm rounded-xl p-4 mb-6 border border-yellow-400/30 bg-yellow-500/10">
                ...
                <div className="space-y-2 text-xs text-yellow-100">
                  {debugInfo.hash_string && (
                    <div>
                      • Hash: {debugInfo.hash_string.substring(0, 20)}...
                    </div>
                  )}
            ...
            {apiResponse && (
              ...
              >
                ...
                >
                  ...
                  {apiResponse.token && (
                    <div>• Token: {apiResponse.token.substring(0, 20)}...</div>
                  )}
                  {apiResponse.test_card_detected && (
                    <div>• Test Kartı: Algılandı</div>
                  )}
                </div>
              </div>
            )}
